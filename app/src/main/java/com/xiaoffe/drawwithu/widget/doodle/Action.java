...
class MyPic extends Action {
    ...
    private RectF rectF;
    MyPic(Bitmap bitmap, Paint paint) {
        mAskBitmap = bitmap;
        mPaint = paint;
    }

    MyPic(Bitmap bitmap, Paint paint, float width, float height){
        mAskBitmap = bitmap;
        mPaint = paint;
    ...
    public void draw(Canvas canvas) {
        if (mAskBitmap != null) {
            //把图片画出来，图片路径是全局的
            //rectF好像可以是null。。不会出问题
            if(rectF != null){
                canvas.drawBitmap(mAskBitmap, null, rectF, mPaint);
            }else{
